...
| G | 相対列幅 | ✅ 実装済み |
| —  | **rtftable 複数 data.frame 対応** | ✅ 実装済み（2026-05-22）|

### rtftable 複数 data.frame 仕様（2026-05-22 確定）

`rtftable$new(data = list(df1, df2, ...), col_header = ...)` で複数 data.frame を受け取れる。

**col_header の解釈ルール**:
- `NULL` → 各 DF の列名をそのままヘッダーとして使用
- 文字スカラー / 文字ベクタ → 全 DF 共通のシングル行ヘッダー
- `list` の長さが DF 数と一致し、各要素が文字ベクタ → 各 DF への個別ヘッダー
- `list` の長さが DF 数と一致しない → 共有の複数行ヘッダー（全 DF に適用）

**制約**:
- 全 DF の列数が一致していること（異なる場合はエラー）
- `col_spec` / `border` / 行高などは全 DF に共通適用
- `spanning_header` は各 DF の先頭で繰り返し出力
- `blank_rows` は各 DF 内で独立に適用

---

## 推奨実装順序（次フェーズ）

1. ~~**H-3** RTF ページフィールド~~ ✅ 実装済み（2026-05-22）
2. ~~**H-2** `assemble_rtf()`~~ ✅ 実装済み（2026-05-22）
3. ~~**I-1** `text_width()`~~ ✅ 実装済み（2026-05-22）
