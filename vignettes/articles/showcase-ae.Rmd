...
> `generate_rtfreport()` -- to write its `.rtf`. (A few sibling blocks share a
> small sort rule or ordering vector, each defined at its first use and flagged
> there.) We render with `eval = FALSE` in the
...
restricted to the three randomised arms. The orderings and filters that only
*some* frameworks need are defined further down, beside the blocks that use them.
...
AE_MAX_ROWS <- 30L   # rows per page; tuned so page 1 fits when Word re-renders
...
# fmt_ae(): cell formatter used by every framework EXCEPT tern (which formats its
# own zeros). Collapse a zero cell "0 (0.0%)" to a bare "0" (so SOC and PT zeros
# match tern), then align.
...
takes one rule **per hierarchy level** (`AE_SORT`, defined at the top of this
block: SOCs alphabetical, PTs by subject count, ties A -> Z). The overall row is
labelled to match tern via the native `label` argument, and `fmt_ae()` collapses
a zero cell to a bare `0`.

```{r ae-gtsummary}
library(gtsummary)
# AE_SORT: the native per-hierarchy-level sort rule -- SOCs alphanumeric, PTs by
# total distinct-subject count (all arms) descending, ties A -> Z. Shared by the
# gtsummary-family blocks: this one, cards + gtsummary, and flextable / huxtable.
...
```{r ae-tplyr}
library(Tplyr)
# These three are needed only by the transpose-and-set blocks -- this one and
# cards + tfrmt below -- which assemble the body themselves:
...
# The same gtsummary table as the gtsummary section above (AE_SORT comes from the
# gtsummary block).
g <- tbl_hierarchical(
    data = adae, variables = c(AESOC, AEDECOD), by = TRT01A,
    denominator = adsl, id = USUBJID, overall_row = TRUE,
